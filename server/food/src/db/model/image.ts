...
interface IImageModel extends Model<ImageDocument, {}, IImageMethods> {
    findByMd5(md5: string): Promise<HydratedDocument<ImageDocument, IImageMethods> | null>
    findByBase64(base64: string): Promise<HydratedDocument<ImageDocument, IImageMethods> | null>
}

const imageSchema = new Schema<ImageDocument, IImageModel, IImageMethods>({
    md5: {
        type: String,
        require: true,
        index: true
...
export const Image = model<ImageDocument, IImageModel>('Image', imageSchema, collections.image);
